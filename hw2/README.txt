...
My general approach to this entire assignment was to:
- As far as possible, break up the code into small testable bits
	- This is motivated by the inherent difficulty of debugging/testing the entire process end-to-end and knowing whether it works.
- Keep things simple
- Optimise for performance at the end

The approach for indexing was to abuse the .seek and .write I/O operations to write to the postings file as often as possible (after each term). This made things a lot simpler at seemingly little cost to performance.

By writing to the postings file after each term, we reduce the problem to use basic linked list semantics (which could then be thoroughly unit tested.)

The approach for searching was to convert the query to prefix notation. Then evaluate the query and let recursion take care of the rest.

A significant optimisation was done for AND NOT queries to avoid holding the entire doc_ids list as far as possible (unavoidable for simple NOT queries.)

The recursive approach traded simplcity for a performance overhead that was mitigated by the cache decorators (almost halving the times when profiled)
